...
static int msm_sensor_platform_remove(struct platform_device *pdev)
{
	...
	legacy_n_msm_sensor_free_sensor_data(s_ctrl);
			...
			(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	if (0 == s_ctrl->bypass_video_node_creation) {
		rc = legacy_n_camera_init_v4l2(&client->dev, &session_id);
		if (rc < 0) {
	...
	rc = legacy_n_msm_sd_register(&s_ctrl->msm_sd);
	if (rc < 0) {
		pr_err("failed: legacy_n_msm_sd_register rc %d", rc);
		return rc;
	}
	msm_sensor_v4l2_subdev_fops = v4l2_subdev_fops;
#ifdef CONFIG_COMPAT
	msm_sensor_v4l2_subdev_fops.compat_ioctl32 =
		legacy_n_msm_sensor_subdev_fops_ioctl;
#endif
			...
			(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	if (0 == s_ctrl->bypass_video_node_creation) {
		rc = legacy_n_camera_init_v4l2(&s_ctrl->pdev->dev, &session_id);
		if (rc < 0) {
			pr_err("failed: legacy_n_camera_init_v4l2 rc %d", rc);
			return rc;
		}
	}
	...
	legacy_n_msm_cam_copy_v4l2_subdev_fops(&msm_sensor_v4l2_subdev_fops);
#ifdef CONFIG_COMPAT
	msm_sensor_v4l2_subdev_fops.compat_ioctl32 =
		legacy_n_msm_sensor_subdev_fops_ioctl;
#endif
...
int32_t legacy_n_msm_sensor_driver_probe(void *setting,
	struct msm_sensor_info_t *probed_info, char *entity_name)
{
	...
	if (rc < 0) {
		pr_err("failed: legacy_n_msm_camera_get_dt_power_setting_data rc %d",
			rc);
		goto free_camera_info;
	}
	...
	if (rc < 0) {
		pr_err("failed: legacy_n_msm_camera_fill_vreg_params for PDOWN rc %d",
			rc);
		goto free_camera_info;
	}
...
static int32_t msm_sensor_driver_get_dt_data(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	/* Read subdev info */
	rc = legacy_n_msm_sensor_get_sub_module_index(of_node, &sensordata->sensor_info);
	if (rc < 0) {
		pr_err("failed");
		goto FREE_SENSOR_DATA;
	}
	...
	rc = legacy_n_msm_camera_get_dt_vreg_data(of_node,
		&sensordata->power_info.cam_vreg,
		&sensordata->power_info.num_vreg);
	if (rc < 0) {
		pr_err("failed: legacy_n_msm_camera_get_dt_vreg_data rc %d", rc);
		goto FREE_SUB_MODULE_DATA;
	}

	/* Read gpio information */
	rc = legacy_n_msm_sensor_driver_get_gpio_data
		(&(sensordata->power_info.gpio_conf), of_node);
	if (rc < 0) {
		pr_err("failed: legacy_n_msm_sensor_driver_get_gpio_data rc %d", rc);
		goto FREE_VREG_DATA;
	}
...
static int32_t msm_sensor_driver_parse(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	rc = legacy_n_msm_sensor_init_default_params(s_ctrl);
	if (rc < 0) {
		pr_err("failed: legacy_n_msm_sensor_init_default_params rc %d", rc);
		goto FREE_DT_DATA;
	}
...
static int32_t msm_sensor_driver_platform_probe(struct platform_device *pdev)
{
	...
	rc = legacy_n_msm_camera_get_clk_info(s_ctrl->pdev,
	...
	const struct i2c_device_id *id)
{
	...
	if (s_ctrl->sensor_i2c_client != NULL) {
		...
		rc = legacy_n_msm_camera_i2c_dev_get_clk_info(
			...
			&s_ctrl->sensordata->power_info.clk_info_size);
		if (rc < 0) {
			pr_err("failed: legacy_n_msm_camera_i2c_dev_get_clk_info rc %d",
				rc);
			goto FREE_S_CTRL;
		}
	}
