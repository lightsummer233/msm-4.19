...
int legacy_n_msm_ispif_get_clk_info(struct ispif_device *ispif_dev,
...
static void msm_ispif_io_dump_reg(struct ispif_device *ispif)
{
	...
	legacy_n_msm_camera_io_dump(ispif->base, 0x250, 0);
}
...
static int msm_ispif_reset_hw(struct ispif_device *ispif)
{
	...
	rc = legacy_n_msm_camera_clk_enable(&ispif->pdev->dev,
	...
	legacy_n_msm_camera_io_w(ISPIF_RST_CMD_MASK,
	...
	if (ispif->hw_num_isps > 1) {
		...
		legacy_n_msm_camera_io_w(ISPIF_RST_CMD_1_MASK,
...
clk_disable:
	if (ispif->clk_idx == 1) {
		rc = rc ? rc : legacy_n_msm_camera_clk_enable(&ispif->pdev->dev,
			ispif->clk_info, ispif->clks,
			ispif->num_clk, 0);
	}
	...
	struct platform_device *pdev)
{
	...
	rc = legacy_n_msm_camera_get_clk_info(pdev, &clk_info,
			&clks, &num_clks);
	...
	return 0;
alloc_fail:
	legacy_n_msm_camera_put_clk_info(pdev, &clk_info, &clks, num_clks);
	return rc;
}
...
static int msm_ispif_clk_ahb_enable(struct ispif_device *ispif, int enable)
{
	...
	rc = legacy_n_msm_cam_clk_enable(&ispif->pdev->dev,
...
static int msm_ispif_reset(struct ispif_device *ispif)
{
	...
	for (i = 0; i < ispif->vfe_info.num_vfe; i++) {
		...
		legacy_n_msm_camera_io_w(1 << PIX0_LINE_BUF_EN_BIT,
			ispif->base + ISPIF_VFE_m_CTRL_0(i));
		legacy_n_msm_camera_io_w(0, ispif->base + ISPIF_VFE_m_IRQ_MASK_0(i));
		legacy_n_msm_camera_io_w(0, ispif->base + ISPIF_VFE_m_IRQ_MASK_1(i));
		legacy_n_msm_camera_io_w(0, ispif->base + ISPIF_VFE_m_IRQ_MASK_2(i));
		legacy_n_msm_camera_io_w(0xFFFFFFFF, ispif->base +
		...
		legacy_n_msm_camera_io_w(0, ispif->base + ISPIF_VFE_m_INPUT_SEL(i));

		legacy_n_msm_camera_io_w(ISPIF_STOP_INTF_IMMEDIATELY,
			ispif->base + ISPIF_VFE_m_INTF_CMD_0(i));
		legacy_n_msm_camera_io_w(ISPIF_STOP_INTF_IMMEDIATELY,
			ispif->base + ISPIF_VFE_m_INTF_CMD_1(i));
		pr_debug("%s: base %pK", __func__, ispif->base);
		legacy_n_msm_camera_io_w(0, ispif->base +
	...
	legacy_n_msm_camera_io_w_mb(ISPIF_IRQ_GLOBAL_CLEAR_CMD, ispif->base +
	...
	uint8_t intftype, uint8_t csid, uint8_t vfe_intf)
{
	...
	data = legacy_n_msm_camera_io_r(ispif->base + ISPIF_VFE_m_INPUT_SEL(vfe_intf));
	switch (intftype) {
	case PIX0:
		data &= ~(BIT(1) | BIT(0));
		data |= (uint32_t) csid;
	...
	legacy_n_msm_camera_io_w_mb(data, ispif->base +
	...
	uint16_t end_pixel)
{
	...
	data = legacy_n_msm_camera_io_r(ispif->base + ISPIF_VFE_m_CTRL_0(vfe_intf));
	data |= (1 << (intftype + 7));
	if (intftype == PIX0)
		data |= 1 << PIX0_LINE_BUF_EN_BIT;
	legacy_n_msm_camera_io_w(data,
		ispif->base + ISPIF_VFE_m_CTRL_0(vfe_intf));

	if (intftype == PIX0)
		legacy_n_msm_camera_io_w_mb(start_pixel | (end_pixel << 16),
	...
	uint8_t intftype, uint16_t cid_mask, uint8_t vfe_intf, uint8_t enable)
{
	...
	data = legacy_n_msm_camera_io_r(ispif->base + intf_addr);
	if (enable)
		data |=  (uint32_t) cid_mask;
	else
		data &= ~((uint32_t) cid_mask);
	legacy_n_msm_camera_io_w_mb(data, ispif->base + intf_addr);
}
	...
	uint8_t intftype, uint8_t vfe_intf)
{
	...
	switch (intftype) {
	case PIX0:
		data = legacy_n_msm_camera_io_r(ispif->base +
	...
	uint8_t intftype, uint8_t csid, uint8_t vfe_intf)
{
	...
	switch (intftype) {
	case PIX0:
		data = legacy_n_msm_camera_io_r(ispif->clk_mux_base);
		data &= ~(0xf << (vfe_intf * 8));
		data |= (csid << (vfe_intf * 8));
		legacy_n_msm_camera_io_w(data, ispif->clk_mux_base);
		break;
	...
	void *data)
{
	...
	for (i = 0; i < params->num; i++) {
		...
		legacy_n_msm_camera_io_w(0x0, ispif->base +
			ISPIF_VFE_m_IRQ_MASK_0(vfe_intf));
		legacy_n_msm_camera_io_w(0x0, ispif->base +
			ISPIF_VFE_m_IRQ_MASK_1(vfe_intf));
		legacy_n_msm_camera_io_w_mb(0x0, ispif->base +
	...
	for (vfe_intf = 0; vfe_intf < 2; vfe_intf++) {
		legacy_n_msm_camera_io_w(ISPIF_IRQ_STATUS_MASK, ispif->base +
			ISPIF_VFE_m_IRQ_MASK_0(vfe_intf));

		legacy_n_msm_camera_io_w(ISPIF_IRQ_STATUS_MASK, ispif->base +
			ISPIF_VFE_m_IRQ_CLEAR_0(vfe_intf));

		legacy_n_msm_camera_io_w(ISPIF_IRQ_STATUS_1_MASK, ispif->base +
			ISPIF_VFE_m_IRQ_MASK_1(vfe_intf));

		legacy_n_msm_camera_io_w(ISPIF_IRQ_STATUS_1_MASK, ispif->base +
			ISPIF_VFE_m_IRQ_CLEAR_1(vfe_intf));

		legacy_n_msm_camera_io_w(ISPIF_IRQ_STATUS_2_MASK, ispif->base +
			ISPIF_VFE_m_IRQ_MASK_2(vfe_intf));
...
static void msm_ispif_config_stereo(struct ispif_device *ispif,
	...
	for (i = 0; i < params->num; i++) {
		...
		if (params->entries[i].intftype == PIX0 &&
			...
			legacy_n_msm_camera_io_w_mb(0x3,
			...
			if (use_line_width &&
				...
				stereo_3d_threshold =
					(params->line_width[vfe_intf] +
							2 * 6 - 1) / (2 * 6);
			legacy_n_msm_camera_io_w_mb(stereo_3d_threshold,
				ispif->base +
	...
	struct msm_ispif_param_data_ext *params)
{
	...
	if (vfe_mask & (1 << VFE0)) {
		atomic_set(&ispif->reset_trig[VFE0], 1);
		/* initiate reset of ISPIF */
		legacy_n_msm_camera_io_w(ISPIF_RST_CMD_MASK_RESTART,
				ispif->base + ISPIF_RST_CMD_ADDR);
		timeout = wait_for_completion_timeout(
			&ispif->reset_complete[VFE0], msecs_to_jiffies(500));
		if (timeout <= 0) {
	...
	if (ispif->hw_num_isps > 1  && (vfe_mask & (1 << VFE1))) {
		atomic_set(&ispif->reset_trig[VFE1], 1);
		legacy_n_msm_camera_io_w(ISPIF_RST_CMD_1_MASK_RESTART,
			ispif->base + ISPIF_RST_CMD_1_ADDR);
		timeout = wait_for_completion_timeout(
				&ispif->reset_complete[VFE1],
				msecs_to_jiffies(500));
...
disable_clk:
	legacy_n_msm_camera_clk_enable(&ispif->pdev->dev,
		ispif->clk_info, ispif->clks,
		ispif->num_clk, 0);
disable_regulator:
	/* Turn OFF regulators */
		...
		enum msm_ispif_vfe_intf vfe_id)
{
	...
	pr_info("%s;%d Reconfiguring 3D mode for VFE%d", __func__, __LINE__,
			vfe_id);
	reg_data =  0xFFFCFFFC;
	legacy_n_msm_camera_io_w_mb(reg_data, ispif->base +
	...
	if (vfe_id == VFE0) {
		...
		legacy_n_msm_camera_io_w_mb(reg_data, ispif->base + ISPIF_RST_CMD_ADDR);
	} else {
		reg_data = 0;
		reg_data |= (PIX_0_VFE_RST_STB | PIX_1_VFE_RST_STB |
				STROBED_RST_EN | PIX_0_CSID_RST_STB |
	...
	void *data)
{
	...
	out[VFE0].ispifIrqStatus0 = legacy_n_msm_camera_io_r(ispif->base +
		ISPIF_VFE_m_IRQ_STATUS_0(VFE0));
	legacy_n_msm_camera_io_w(out[VFE0].ispifIrqStatus0,
		ispif->base + ISPIF_VFE_m_IRQ_CLEAR_0(VFE0));

	out[VFE0].ispifIrqStatus1 = legacy_n_msm_camera_io_r(ispif->base +
		ISPIF_VFE_m_IRQ_STATUS_1(VFE0));
	legacy_n_msm_camera_io_w(out[VFE0].ispifIrqStatus1,
		ispif->base + ISPIF_VFE_m_IRQ_CLEAR_1(VFE0));

	out[VFE0].ispifIrqStatus2 = legacy_n_msm_camera_io_r(ispif->base +
		ISPIF_VFE_m_IRQ_STATUS_2(VFE0));
	legacy_n_msm_camera_io_w_mb(out[VFE0].ispifIrqStatus2,
		ispif->base + ISPIF_VFE_m_IRQ_CLEAR_2(VFE0));

	if (ispif->vfe_info.num_vfe > 1) {
		out[VFE1].ispifIrqStatus0 = legacy_n_msm_camera_io_r(ispif->base +
			ISPIF_VFE_m_IRQ_STATUS_0(VFE1));
		legacy_n_msm_camera_io_w(out[VFE1].ispifIrqStatus0,
			ispif->base + ISPIF_VFE_m_IRQ_CLEAR_0(VFE1));

		out[VFE1].ispifIrqStatus1 = legacy_n_msm_camera_io_r(ispif->base +
			ISPIF_VFE_m_IRQ_STATUS_1(VFE1));
		legacy_n_msm_camera_io_w(out[VFE1].ispifIrqStatus1,
				ispif->base + ISPIF_VFE_m_IRQ_CLEAR_1(VFE1));

		out[VFE1].ispifIrqStatus2 = legacy_n_msm_camera_io_r(ispif->base +
			ISPIF_VFE_m_IRQ_STATUS_2(VFE1));
		legacy_n_msm_camera_io_w_mb(out[VFE1].ispifIrqStatus2,
	...
	if ((out[VFE0].ispifIrqStatus0 &  PIX_INTF_0_OVERFLOW_IRQ) ||
		...
		reg_data = legacy_n_msm_camera_io_r(ispif->base +
	...
	uint32_t csid_version)
{
	...
	rc = legacy_n_msm_camera_enable_irq(ispif->irq, 1);
	if (rc < 0) {
	...
	if (ispif->csid_version >= CSID_VERSION_V30 && !ispif->clk_mux_base) {
		ispif->clk_mux_base = legacy_n_msm_camera_get_reg_base(ispif->pdev,
							"csi_clk_mux", 1);
...
error_ahb:
	if (legacy_n_cam_config_ahb_clk(NULL, 0, CAM_AHB_CLIENT_ISPIF,
...
static void msm_ispif_release(struct ispif_device *ispif)
{
	...
	legacy_n_msm_camera_enable_irq(ispif->irq, 0);
...
static int ispif_probe(struct platform_device *pdev)
{
	...
	rc = legacy_n_msm_ispif_get_clk_info(ispif, pdev);
	if (rc < 0) {
	...
	ispif->base = legacy_n_msm_camera_get_reg_base(pdev, "ispif", 1);
	if (!ispif->base) {
		rc = -ENOMEM;
		goto reg_base_fail;
	}

	ispif->irq = legacy_n_msm_camera_get_irq(pdev, "ispif");
	if (!ispif->irq) {
		rc = -ENODEV;
		goto get_irq_fail;
	}
	rc = legacy_n_msm_camera_register_irq(pdev, ispif->irq, msm_io_ispif_irq,
			IRQF_TRIGGER_RISING, "ispif", ispif);
	if (rc) {
		rc = -ENODEV;
		goto get_irq_fail;
	}
	rc = legacy_n_msm_camera_enable_irq(ispif->irq, 0);
	if (rc)
		goto sd_reg_fail;
	...
	rc = legacy_n_msm_sd_register(&ispif->msm_sd);
	if (rc) {
		pr_err("%s: legacy_n_msm_sd_register error = %d\n", __func__, rc);
		goto sd_reg_fail;
	}
	legacy_n_msm_cam_copy_v4l2_subdev_fops(&msm_ispif_v4l2_subdev_fops);
...
sd_reg_fail:
	legacy_n_msm_camera_unregister_irq(pdev, ispif->irq, ispif);
get_irq_fail:
	legacy_n_msm_camera_put_reg_base(pdev, ispif->base, "ispif", 1);
reg_base_fail:
	legacy_n_msm_camera_put_clk_info(pdev, &ispif->ahb_clk_info,
		&ispif->ahb_clk,
