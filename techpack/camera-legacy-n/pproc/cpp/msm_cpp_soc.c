...
void legacy_n_msm_cpp_fetch_dt_params(struct cpp_device *cpp_dev)
{
	int rc = 0;
...
int legacy_n_msm_cpp_get_clock_index(struct cpp_device *cpp_dev, const char *clk_name)
{
	uint32_t i = 0;
...
int legacy_n_msm_cpp_set_micro_clk(struct cpp_device *cpp_dev)
{
	int rc;
...
int legacy_n_msm_update_freq_tbl(struct cpp_device *cpp_dev)
{
	uint32_t msm_cpp_core_clk_idx;
	int rc = 0;

	msm_cpp_core_clk_idx = legacy_n_msm_cpp_get_clock_index(cpp_dev, "cpp_core_clk");
...
long legacy_n_msm_cpp_set_core_clk(struct cpp_device *cpp_dev, long rate, int idx)
{
	long rc = 0;

	rc = legacy_n_msm_camera_clk_set_rate(&cpp_dev->pdev->dev,
...
int legacy_n_msm_cpp_read_payload_params_from_dt(struct cpp_device *cpp_dev)
{
